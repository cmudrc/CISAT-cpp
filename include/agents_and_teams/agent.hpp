...
public:
    ...
    std::vector< std::vector<long double> > move_oper_pref; // preference for different move operators, emission probability if hidden markov
    std::vector< std::vector<long double> > trans_prob; // used only for hidden markov, define transition probabilities between states
